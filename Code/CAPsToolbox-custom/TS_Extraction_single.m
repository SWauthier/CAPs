function TS = TS_Extraction_single(data,V,seed_mni,seed_radius)
...
for j=1:num_seed
    ...
    tmp=data;
    [seed_cor] = Comp_Ball2Mask(V(1).dim,abs(diag(V(1).mat(1:3,1:3)))',seed_mni(j,:), seed_radius, V(1));
    seed_ind = sub2ind(V(1).dim, seed_cor(:,1), seed_cor(:,2), seed_cor(:,3));
    TS{1,j} = mean(tmp(:,seed_ind),2);
