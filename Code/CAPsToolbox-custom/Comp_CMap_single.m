function [CMap, TS] = Comp_CMap_single(data,V,brind,seed_mni,seed_name,seed_radius)  
...
for iseed=1:num_seed
    ...
    TS{1,iseed} = mean(Y(:,seed_ind),2);
    
    X = mean(data{1}(:,seed_ind),2);
    fprintf('\n Number of NaN in the seed %d \n',length(find(isnan(X)))); 
    dat = zeros(V(1).dim);
    tmp = Comp_fastCorr(X, Y(:,brind)); 
    dat(brind) = tmp;
